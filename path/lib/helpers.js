export const SKILL_CATEGORIES=['Digital & Data','Gestione Progetti','Comunicazione','Leadership','Tecniche di Settore','Analisi & Problem Solving','Creatività & Design','Altro'];
...
export function makePublicSlug({nome,cognome,email}){const base=slugify(`${nome||''} ${cognome||''}`.trim())||slugify(email||'qrcv');return `${base}-${Math.random().toString(16).slice(2,8)}`}
