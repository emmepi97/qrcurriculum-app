...
export default function PublicQrCvPage() {
    ...
    async function load() {
      try {
        ...
        if (!tracked.current) {
          ...
          await supabase.from('analytics_events').insert({ ...base, event_type: 'profile_view' });
          if (params.get('qr') === '1') await supabase.from('analytics_events').insert({ ...base, event_type: 'qr_scan' });
        }
      } catch (e) {
  ...
  async function downloadPdf() {
    // Cattura direttamente il foglio mostrato in pagina: anteprima e
    // PDF sono garantiti identici perché sono lo stesso nodo DOM.
    await exportCvPdf(cvRef.current, 'CV.pdf');
  }

  if (loading) return <p style={{ padding: 30 }}>{t.loading}</p>;
  if (error) return <p className="error" style={{ padding: 30 }}>{error}</p>;
