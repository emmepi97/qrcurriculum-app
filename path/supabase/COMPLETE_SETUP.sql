...
--
-- Non serve eseguire separatamente: schema.sql, migration_v16...v21.
...
create table if not exists public.projects (id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade, title text not null, role text default '', description text default '', start_date date, end_date date, links jsonb not null default '[]'::jsonb, hide_description boolean not null default false, is_hidden boolean not null default false, content_lang text not null default 'all', cv_visibility jsonb not null default '{}'::jsonb, created_at timestamptz not null default now());
...
-- ============ 4) FIX v18: colonna owner_id legacy su analytics_events ============
...
-- ============ 5) FIX v19: Job Match AI opzionale ============
...
-- ============ 6) FIX v20: allegati/link nelle sezioni ============
...
-- ============ 7) FIX v21: slot sponsorizzati corsi (Job Match) ============
