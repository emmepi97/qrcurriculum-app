...
create table if not exists public.personal_info (
  ...
  public_slug text unique not null, is_public boolean not null default true,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique(user_id)
);
create table if not exists public.work_experiences (id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade, role_title text not null, company text default '', start_date date, end_date date, description text default '', hide_description boolean not null default false, is_hidden boolean not null default false, content_lang text not null default 'all', cv_visibility jsonb not null default '{}'::jsonb, created_at timestamptz not null default now());
create table if not exists public.educations (id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade, title text not null, institution text default '', field_of_study text default '', start_date date, end_date date, location text default '', grade text default '', description text default '', hide_description boolean not null default false, is_hidden boolean not null default false, content_lang text not null default 'all', cv_visibility jsonb not null default '{}'::jsonb, created_at timestamptz not null default now());
...
create table if not exists public.awards (id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade, title text not null, issuer text default '', date date, description text default '', hide_description boolean not null default false, is_hidden boolean not null default false, content_lang text not null default 'all', cv_visibility jsonb not null default '{}'::jsonb, created_at timestamptz not null default now());
create table if not exists public.projects (id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade, title text not null, role text default '', description text default '', start_date date, end_date date, hide_description boolean not null default false, is_hidden boolean not null default false, content_lang text not null default 'all', cv_visibility jsonb not null default '{}'::jsonb, created_at timestamptz not null default now());
create table if not exists public.case_studies (id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade, title text not null, context text default '', solution text default '', impact text default '', date date, hide_description boolean not null default false, is_hidden boolean not null default false, content_lang text not null default 'all', cv_visibility jsonb not null default '{}'::jsonb, created_at timestamptz not null default now());

create table if not exists public.analytics_events (id uuid primary key default gen_random_uuid(), profile_user_id uuid references auth.users(id) on delete cascade, event_type text not null check (event_type in ('profile_view','qr_scan')), public_slug text not null, path text default '', referrer text default '', user_agent text default '', created_at timestamptz not null default now());
...
-- V19 portfolio video presentation
