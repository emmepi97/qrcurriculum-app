...
Aggiunto anche `index.html` nella root del progetto per compatibilita con sistemi che lo richiedono esplicitamente. In Next.js App Router la home ufficiale resta `app/page.js`.


## V12.2 - Layout dashboard + PDF export fix

- Forzata la dashboard a occupare tutta la larghezza disponibile del browser.
- Rimossa la sensazione di app compressa al centro della pagina.
- Preview CV piu ampia e utilizzabile.
- Download PDF ora usa un template hidden dedicato, non la preview scalata/scrollabile.
- PDF export compatto per rientrare meglio in una singola pagina A4.
- Corretto il taglio a destra: export con larghezza A4 controllata, box-sizing e overflow hidden.

## V14.1 Complete Project Fixed
Questo ZIP contiene il progetto completo ricostruito dal bundle completo, incluse le cartelle app, components, lib e supabase.
Correzioni applicate senza rimuovere funzionalita':
- rimosso definitivamente contorno/bordo/ombra dal CV tramite override finale in app/globals.css;
- dashboard/editor forzati a larghezza piena tramite override finale in app/globals.css;
- mantenute tutte le pagine, lib e migration Supabase presenti nel bundle completo.
