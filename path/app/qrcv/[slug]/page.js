    ...
    async function load() {
      try {
        ...
        if (!tracked.current) {
          ...
          const { error: viewErr } = await supabase.from('analytics_events').insert({ ...base, event_type: 'profile_view' });
          if (viewErr) console.error('Analytics tracking error (profile_view):', viewErr.message);
          if (params.get('qr') === '1') {
            const { error: scanErr } = await supabase.from('analytics_events').insert({ ...base, event_type: 'qr_scan' });
            if (scanErr) console.error('Analytics tracking error (qr_scan):', scanErr.message);
          }
        }
      } catch (e) {
