...
function axisPoint(i, total, size = 360, r = 138) {
  const a = (-90 + i * (360 / total)) * Math.PI / 180;
  return [size / 2 + r * Math.cos(a), size / 2 + r * Math.sin(a)];
}
function polygon(values, size = 360) {
  const cx = size / 2, cy = size / 2, maxR = 138;
        ...
        <div className="radarTop">
          <div className="radarChartCard">
            ...
            <div className="radarSvgWrap">
              <svg viewBox="0 0 360 360" className="radarSvg">
                {[1, 2, 3, 4, 5].map(l => (
                  <polygon key={l} className="radarGrid" points={SKILL_CATEGORIES.map((_, i) => axisPoint(i, SKILL_CATEGORIES.length, 360, 138 * l / 5).join(',')).join(' ')} />
                ))}
                {SKILL_CATEGORIES.map((c, i) => {
                  const [x, y] = axisPoint(i, SKILL_CATEGORIES.length);
                  return (
                    <g key={c}>
                      <line className="radarAxis" x1="180" y1="180" x2={x} y2={y} />
                      <text className="radarLabel" x={x} y={y} textAnchor={x > 190 ? 'start' : x < 170 ? 'end' : 'middle'}>
                ...
                {vals.map((v, i) => {
                  const [x, y] = axisPoint(i, SKILL_CATEGORIES.length, 360, 138 * v / 5);
        ...
        <section className="smartSection">
          ...
          <div className="suggestionGrid">
            {insights.suggestions.length === 0 ? (
              ...
              <article className="suggestionCard" key={s.category}>
