...
export default function MonetizationPanel({userId}){
  ...
  return <section className="smartSection wideSection"><div className="smartSectionHeader"><div><h2>{t.monetization}</h2><p>{lang==='en'?'Premium, availability and portfolio reviews.':'Premium, disponibilità e recensioni nel portfolio.'}</p><div className="hint monetizationExplain"><b>{lang==='en'?'Payment structure':'Struttura a pagamento'}</b><p>{lang==='en'?'Free users keep the public CV, QR code and basic totals. Premium unlocks advanced analytics after the trial, multiple CV versions and professional features such as visibility in the directory and portfolio reviews. Stripe is not connected yet: the button is only a placeholder, while plan/status can currently be managed from Supabase.' :'Gli utenti Free mantengono CV pubblico, QR code e totali base. Premium sblocca gli analytics avanzati dopo il trial, CV multipli e funzioni professionali come presenza in directory e recensioni portfolio. Stripe non è ancora collegato: il pulsante è solo predisposto, mentre piano e stato per ora si gestiscono da Supabase.'}</p></div></div></div>
