...
function LinkChips({ links }) {
  ...
  return (
    <div className="pfLinkChips" onClick={e => e.stopPropagation()}>
      {links.map((l, i) => (
        <a key={i} className="pfLinkChip" href={l.url} target="_blank" rel="noreferrer">
          🔗 {l.label || l.url}
        </a>
      ))}
    </div>
...
function ExpandableItem({ heading, subheading, date, description, links, skills }) {
  const [open, setOpen] = useState(false);
  const hasLinks = Array.isArray(links) && links.length > 0;
  const hasSkills = Array.isArray(skills) && skills.length > 0;
  const expandable = Boolean(description) || hasLinks || hasSkills;
  return (
      ...
      >
        ...
        {expandable && open && (
          <>
            ...
            {hasSkills && <div className="pfSkillTags pfLinkedSkills">{skills.map(sk => <span className="tagPill" key={sk}>{sk}</span>)}</div>}
...
function ExpandableCard({ tag, title, meta, date, children, hasDetail, links, skills }) {
  const [open, setOpen] = useState(false);
  const hasLinks = Array.isArray(links) && links.length > 0;
  const hasSkills = Array.isArray(skills) && skills.length > 0;
  const canExpand = hasDetail || hasLinks || hasSkills;
  return (
    <article
      className={`pfProjectCard ${canExpand ? 'pfExpandable' : ''}`}
    ...
    >
      ...
      <h3>{title}{canExpand && <span className="pfExpandIcon">{open ? '−' : '+'}</span>}</h3>
...
export default function PortfolioView({ data, qrCode, lang, worker, publicUrl }) {
  ...
  const hasAvailability = availability && (availability.consulting || availability.freelance || availability.networking || availability.teaching || availability.collaborations);
          ...
          <div className="pfTimeline">
            {work.map(x => (
              <ExpandableItem
                ...
                links={x.links}
                skills={fieldVisible(x, 'linked_skills') ? x.linked_skills : []}
              />
            ))}
          </div>
        )}
