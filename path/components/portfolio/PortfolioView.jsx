...
function ExpandableItem({ heading, subheading, date, description, links }) {
  const [open, setOpen] = useState(false);
  const hasLinks = Array.isArray(links) && links.length > 0;
  const expandable = Boolean(description) || hasLinks;
  return (
...
function ExpandableCard({ tag, title, meta, date, children, hasDetail, links }) {
  const [open, setOpen] = useState(false);
  const hasLinks = Array.isArray(links) && links.length > 0;
  const canExpand = hasDetail || hasLinks;
  return (
    <article
